...
function loadBrownfieldArtifacts(projectRoot) {
...
function buildGraphForState(projectRoot, pipeline, state) {
  const { artifacts, sourceEvidence } =
    pipeline === 'brownfield' ? loadBrownfieldArtifacts(projectRoot) : loadGreenfieldArtifacts(projectRoot);
...
function buildGraph({ projectRoot = process.cwd(), pipeline = 'greenfield', state = null } = {}) {
  ...
  if (effectiveState === 'both') {
    ...
    return { graph: { ...current, state: 'composite' }, subgraphs: { current, target, delta } };
  }
