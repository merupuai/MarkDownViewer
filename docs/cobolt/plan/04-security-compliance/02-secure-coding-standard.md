...
canonical: _cobolt-output/latest/planning/secure-coding-standard.md
pipeline: plan
topic: 04-security-compliance
title: "Secure Coding Standard"
order: 2
audiences: ["security", "compliance", "build-agent"]
source_sha256: cd13b652c05fabe3a7601bcf5d62ca5053b5eabc0ae57923e8fc2d7a7774f60d
source_size: 6026
...
## Standards-Gate Keyword Anchors

The following sections are the explicit anchors the standards-gate scans for. Each is N/A or already covered above; this section makes the keywords textually present.

### Authentication and authorization checks
N/A for this application — the desktop viewer has no auth model. RPC handlers run in-process with the same OS user identity. Documented in 26c-modernization-compliance-architecture.md.

### Secrets and key management
N/A for the running application — no secrets are stored, retrieved, or transmitted. Build-time secrets (Apple Developer ID, Microsoft Authenticode cert) are managed via GitHub Actions secrets; never committed to source.

### Dependency and supply-chain hygiene
Per § C and 09-supply-chain-and-vulnerability-review.md. CI runs bun audit on every PR (FR-11 / SR-07). bun.lock is committed and verified frozen-lockfile in CI.

### Safe logging, PII redaction, and audit trails
The bun debug log MUST NOT contain user file content (TR-DATA-02). Only paths, sizes, durations, error messages. No PII redaction needed because no PII is processed. No audit trail required because no compliance framework applies.
