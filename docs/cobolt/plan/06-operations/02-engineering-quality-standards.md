...
canonical: _cobolt-output/latest/planning/engineering-quality-standards.md
pipeline: plan
topic: 06-operations
title: "Engineering Quality Standards"
order: 2
audiences: ["sre", "ops", "qa", "platform-lead"]
source_sha256: 3819615f46a01999df578fbb976c7a47448c80127dd7165d1b179a5d09e8c644
source_size: 4729
...
### API, schema, and contract rules
The single API surface is the in-process RPC schema in src/shared/rpc.ts. Versioning strategy: discriminated-union response shapes for forward-compat; optional request fields for additive changes. See 30-modernization-api-contracts.md § Versioning Strategy.

### Maintainability and modularity guidance
- One module per concept (markdown.ts pure; index.ts orchestration; sanitize.ts post-M1)
- Keep mainview/index.ts under 1000 LOC; split when exceeded (currently 813)
- Cross-process types live ONLY in src/shared/
- All new modules ship with a unit test in tests/unit/
- Refactor without behavior change requires a green test suite first
